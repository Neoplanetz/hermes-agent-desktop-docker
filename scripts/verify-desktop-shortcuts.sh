...
docker exec "$C" su - "$U" -c 'gio info ~/Desktop/hermes-terminal.desktop 2>/dev/null | grep -q "metadata::trusted: true" && gio info ~/Desktop/hermes-setup.desktop 2>/dev/null | grep -q "metadata::trusted: true"' \
  && echo "  OK both trusted" || { echo "  FAIL not trusted"; exit 1; }
